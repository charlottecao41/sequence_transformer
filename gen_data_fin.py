    ...
    if force_last:
        for user in all_documents:
            instances.extend(
                create_instances_from_document_test(
                    all_documents, user, max_seq_length,movie_items))
        ...
        for step in range(dupe_factor):
            pool.apply_async(
                create_instances_threading, args=(
                    ...
                    mask_prob, step,movie_items), callback=log_result)
        pool.close()
        pool.join()
        ...
        for user in all_documents:
            instances.extend(
                mask_last(
                    all_documents, user, max_seq_length, short_seq_prob,
                    masked_lm_prob, max_predictions_per_seq, vocab, rng,movie_items))
...
def mask_last(
        ...
        max_predictions_per_seq, vocab, rng,movie_items):
    ...
    for tokens in document:
        ...
        (tokens, masked_lm_positions,
         masked_lm_labels) = create_masked_lm_predictions_force_last(tokens,movie_items)
...
def create_instances_from_document_test(all_documents, user, max_seq_length,movie_items):
    """Creates `TrainingInstance`s for a single document."""
    document = all_documents[user]
    max_num_tokens = max_seq_length
...
def create_instances_from_document_train(
        all_documents, user, max_seq_length, short_seq_prob, masked_lm_prob,
        max_predictions_per_seq, vocab, rng, mask_prob,movie_items):
    """Creates `TrainingInstance`s for a single document."""
    document = all_documents[user]
    ...
    for tokens in document:
        ...
        (tokens, masked_lm_positions,
         masked_lm_labels) = create_masked_lm_predictions(
            ...
            vocab_items, rng, mask_prob,movie_items)
        instance = TrainingInstance(
            info=info,
            tokens=tokens,
            masked_lm_positions=masked_lm_positions,
...
def create_masked_lm_predictions_force_last(tokens,movie_items):
...
def create_masked_lm_predictions(tokens, masked_lm_prob,
    ...
    for index in cand_indexes:
        ...
        if index in covered_indexes:
            continue
        if tokens[index] not in movie_items:
            continue
...
def gen_samples(data,
    ...
    instances = create_training_instances(
        ...
        pool_size, movie_items,force_last)
...
def main():
    ...
    dataset = data_partition(output_dir + dataset_name + '.txt', FLAGS.context_length)
    ...
    user_test_data = {
        'user_' + str(u):
            ['item_' + str(item) for item in (user_train[u] + user_test[u])]
        for u in user_train if len(user_test[u]) > 0
    }
    ...
    vocab = FreqVocab(user_test_data2)
    ...
    movie_items = set()
    for u in user_train:
        for item in (user_train[u] + user_test[u]):
            if item < 50000:
                movie_items.add('item_' + str(item))
    movie_items=list(movie_items)
    movie_items_ids = vocab.convert_tokens_to_ids(movie_items)
    ...
    itemid_pool_name = output_dir + dataset_name + version_id + '.item'
    print('item pickle file: ' + itemid_pool_name)
    with open(itemid_pool_name, 'wb') as output_file:
        pickle.dump(movie_items_ids, output_file, protocol=2)
