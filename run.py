...
class EvalHooks(tf.compat.v1.train.SessionRunHook):
    ...
    def begin(self):
        ...
        if FLAGS.vocab_filename is not None:
            ...
            with open(FLAGS.itemid_pool_filename, 'rb') as input_file:
                self.itemidpool = pickle.load(input_file)
    ...
    def end(self, session):
        print(
            "ndcg@1:{}, hit@1:{}, ndcg@50:{}, hit@50:{}, ndcg@10:{}, hit@10:{}, MRR:{}, valid_user:{}".
    ...
    def after_run(self, run_context, run_values):
        ...
        for idx in range(len(input_ids)):
            ...
            print("item_num:"+str(size_of_prob))
            if self.vocab is not None:
                sampled_ids = [x for x in self.itemidpool if x not in rated and x not in item_idx]
    ...
    if FLAGS.do_eval:
        ...
        with tf.compat.v1.gfile.GFile(output_eval_file, "w") as writer:
            ...
            for key in sorted(result.keys()):
                ...
                writer.write("%s = %s\n" % (key, str(result[key])))
    shutil.rmtree(FLAGS.checkpointDir, ignore_errors=True)
    # os.removedirs(FLAGS.checkpointDir)
